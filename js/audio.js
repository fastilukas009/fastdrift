...
export class GameAudio {
  ...
  init() {
    ...
    this.shaper.curve = distortionCurve(2.6);
    ...
    this.oscs = [];
    const harmonics = [
      { mul: 0.5, gain: 0.42, type: 'sawtooth', detune: -6 },
      { mul: 1.0, gain: 0.55, type: 'sawtooth', detune: 4 },
      { mul: 2.0, gain: 0.22, type: 'square', detune: -10 },
      { mul: 3.0, gain: 0.10, type: 'sawtooth', detune: 8 }
    ];
    ...
    this.engineNoiseFilter.frequency.value = 320;
  ...
  backfire() { this.burst({ duration: 0.13, freq: 260, q: 0.7, gain: 0.55, sweep: 0.3 }); }
  ...
  update(dt, vehicle, opts = {}) {
    ...
    // Sytytystaajuus: kierrokset sekunnissa kertaa sylinterien puolikas.
    const order = spec.body.shape === 'muscle' || spec.body.shape === 'race' ? 4 : 2;
    const f0 = Math.max(18, (rpm / 60) * order);
    for (const o of this.oscs) {
      o.osc.frequency.setTargetAtTime(f0 * o.mul, now, 0.02);
    }

    const load = 0.28 + throttle * 0.72;
    const revShare = Math.min(1, rpm / spec.redline);
    const target = opts.muted ? 0 : (0.10 + load * 0.19) * (0.75 + revShare * 0.45);
    this.engineBus.gain.setTargetAtTime(target, now, 0.05);
    this.engineFilter.frequency.setTargetAtTime(420 + revShare * 2600 + throttle * 1400, now, 0.05);

    this.engineNoiseFilter.frequency.setTargetAtTime(180 + revShare * 900, now, 0.06);
    this.engineNoiseGain.gain.setTargetAtTime(0.02 + throttle * 0.055 + revShare * 0.03, now, 0.06);
    ...
    this.squealGain.gain.setTargetAtTime(slip * 0.16 * surfaceMul, now, 0.05);
    ...
    this.windGain.gain.setTargetAtTime(Math.min(0.11, speed / 90 * 0.11), now, 0.12);
