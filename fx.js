...
export class Effects {
  ...
  emitWheel(wheel, x, y, z, vx, vz, surface, dt) {
    ...
    const rate = (this.quality === 'low' ? 16 : 34) * intensity;
    wheel.smokeAcc = (wheel.smokeAcc || 0) + rate * dt;
    while (wheel.smokeAcc >= 1) {
      wheel.smokeAcc -= 1;
      const spread = 0.9;
      const tint = onRoad ? 0.76 : 0.60;
      const dust = !onRoad;
      this.smoke.spawn({
        ...
        grow: 2.4 + Math.random() * 1.8,
        rot: Math.random() * Math.PI,
        spin: (Math.random() - 0.5) * 1.1,
        alpha: (dust ? 0.24 : 0.30) * (0.4 + intensity * 0.7),
        max: 1.3 + Math.random() * 1.3,
